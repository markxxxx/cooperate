...
Usage 
	...
	$length = 10;
	$max_keywords = 10;
	$this->_seo($html)->get_meta_description($length);
	$this->_seo()->get_keywords($max_keywords);


	or
	
	$seo = $this->_seo($html);
	$seo->getMetaDescription(10);
	$seo->getKeywords(10);
...
class Seo {
	...
	function Seo($text=null){//constructor
		$this->setText($text);
	}
	
	///SEO for meta description
	function get_meta_description($len=null){//returns a text with meta description
		$this->setDescriptionLen($len);//by param set len
		return substr($this->getText(), 0, $this->getDescriptionLen());//shrink X chars
	}
	
	function get_keywords($mKw=null){//from the given text
