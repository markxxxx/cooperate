...
<div id="content">
	...
	<div id="content-top">
    <h2 style="width:100%">Contact - {$data.name} 

    {if $user->can_access('contact','delete')}<a class="minibutton right" style="margin-left:10px;" href="/contact/delete/{$data.id}"> Delete</a>{/if}

	{if $user->can_access('reminder','add')}
		<a class="right minibutton colorbox" href="/reminder/add/0/contact/{$data.id}" >Add reminder
		</a>
	{/if}
								...
								<div style="padding:10px">{$data.description}</div>
													...
													<div id="comment_container_{$data.id}" >
														<form action="/comment/add/internship/{$data.id}/" method="POST">
															<textarea id="intern_comment_{$data.id}" 
															class="" rows="5" style="width:98%" cols="25" name="data[comment]" id='note' rows="5"></textarea>
							...
							<div class="box-container" id="event">
								...
								{if $sent_count}
									<div class="box-container" id="sent">
										<h3 class="green">Sent
												<select name="user_id" class="right" id="sent_user_id">
													<option>Sent to</option>
													{section name=inst loop=$sent_from}
														<option value="{$sent_from[inst].id}">{$sent_from[inst].name}</option>
													{/section}
												</select>

											<input type="input" name="search" id="sent_search" class="right" value="Search..." onclick="if(this.value=='Search...') this.value =''; " />
										...
										<div id="sent_table">
											<table class="table-short" style="width:100% !important">
												...
												<tbody>
													{section name=inst loop=$sent_mail}
														<tr {cycle values="class='odd',"}>
															<td >
																{$sent_mail[inst].message_date|date_format}</small>
															</td>
															<td class="col-second">
																<a class="message_view" href="/contact/message_view/{$sent_mail[inst].id}">{$sent_mail[inst].subject|truncate:30}</a>
															</td>
															<td >
																{$sent_mail[inst].name}</small>
															</td>
								...
								{if $received_count}
									...
									<div class="box-container" id="received">
										<h3 class="green">Received

											<select name="user_id" class="right" id="received_user_id">
												<option>Received from</option>
												{section name=inst loop=$received_from}
													<option value="{$received_from[inst].id}">{$received_from[inst].name}</option>
												{/section}
											</select>

											<input type="input" name="search" class="right" id="received_search" value="Search..." onclick="if(this.value=='Search...') this.value =''; " />
										...
										<div id="received_table">
											<table class="table-short" style="width:100% !important">
												...
												<tbody>
													{section name=inst loop=$received_mail}
														<tr {cycle values="class='odd',"}>
															<td >
																{$received_mail[inst].message_date|date_format}</small>
															</td>
															<td class="col-second">
																<a class="message_view" href="/contact/message_view/{$received_mail[inst].id}">{$received_mail[inst].subject|truncate:30}</a>
															</td>
															<td >
																{$received_mail[inst].name}</small>
															</td>
